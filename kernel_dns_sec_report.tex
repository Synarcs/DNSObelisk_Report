...
\abstract{%
Data exfiltration over DNS presents a serious threat, particularly for hyperscalers handling large-scale AI workloads and sensitive data across distributed environments. In 2024, the average cost of such breaches exceeded \$4.8 million. Existing solutions often fall short in preventing these attacks in real time at scale. 
This project develops a scalable framework for real-time DNS exfiltration prevention, designed to counter advanced Command-and-Control (C2) techniques. The system uses lightweight eBPF agents to perform deep packet inspection directly in the Linux kernel, enabling high-speed, per-packet analysis of outbound DNS traffic. A userspace neural network performs real-time lexical analysis to detect obfuscated or malicious payloads. Upon detection, the kernel enforces active defense by immediately terminating malicious processes to contain the threat at the source. The system also exports telemetry — including process identifiers and DNS usage patterns — to Kafka brokers, supporting dynamic domain blacklisting and policy enforcement across nodes without relying on external middleware.
Experimental results demonstrate that the framework scales effectively and disrupts sophisticated DNS exfiltration attempts, including those modeled by top adversary emulation frameworks. It reduces attacker dwell time, blocks lateral movement, and improves threat visibility in complex, distributed environments.
}
...
\subsection{Datasets}
\label{sec:dataset}
The deep learning model is trained using three primary data sources:
\begin{enumerate}[nosep]
    \item \textbf{Benign SLDs from Cisco:} The top 1 million SLDs from Cisco are not used for training, but are loaded into an in-memory LRU cache by the eBPF agent. This avoids unnecessary inference on common, trusted domains and improves performance. The cache is fully reprogrammable from the control plane.
