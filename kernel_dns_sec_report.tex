...
\titleformat{\section}
  {\bfseries\large}{\thesection}{0.70em}{}
...
\Else{
  ...
  \Else{
    ...
    Delete \texttt{redirect\_ts\_verify\_val} from \texttt{dns\_redirect\_ts\_verify\_map} with key=\texttt{kernel\_time\_ns}\;
    \Return \texttt{TC\_ACT\_OK}\;
  }
}
...
The passive mode architecture is broken down as follows. \hyperref[sec:maps]{\emph{Maps}} introduces the key eBPF structures used in this mode, their types, and their roles, as visualized in Figure~\ref{sec:dp_eBPF_LRU_Maps_passive}. \hyperref[passive:sec1]{\emph{Kernel packet processing}} details the eBPF programs in the kernel to identify and drop malicious DNS exfiltration attempts while tracking the responsible userspace processes. It also leverages kernel tracepoints attached to kernel process scheduler for efficient garbage collection of malicious process tracking. The next section, \hyperref[passive:sec2]{\emph{Userspace packet processing}}, describes the eBPF agent that parses and classifies traffic to identify potentially malicious processes involved in exfiltration. Finally, \hyperref[passive:sec3]{\emph{eBPF map concurrency}} explains the kernel and userspace components maintain a consistent eBPF map state under concurrent CPU access.
...
% % alg 5
\begin{algorithm}[H]
\caption{eBPF Map Handling within Kernel for \textbf{Passive} Mode of Agent}
\label{sec:alg5}
\SetKwInOut{Input}{Input}
\SetKwInOut{Output}{Output}
...
\begin{enumerate}[nosep]
    ...
    Table~\ref{dns_payload_obfuscation}) across various file types—text (e.g., \texttt{.txt}, \texttt{.md}), image (e.g., \texttt{.jpg}, \texttt{.jpeg}, \texttt{.png}), and video (e.g., \texttt{.mp4})
\end{enumerate}
...
\centering
\caption{Kafka Topics utilized by the eBPF Agent}
...
\begin{minipage}[t]{0.45\textwidth}
  ...
  \vspace{0.5em}
  \centering
  \small % Reduce font size to help it fit
  \begin{tabular}{@{}lcc@{}} % @{} removes side padding
    \toprule
...
\begin{minipage}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=0.95\textwidth]{UWThesis/images/results/model/confusion_matrix.png}
...
\begin{figure}[H] 
 ...
 \begin{minipage}[b]{1\textwidth}
    \centering
    \includegraphics[width=0.57\textwidth]{UWThesis/images/results/response_speed/response_speed.png}
  ...
  %   \centering
  %   \includegraphics[width=0.5\textwidth]{UWThesis/images/results/response_speed/kernel_dpi_time.png}
  %   \caption{eBPF TC filter DNS DPI time} 
  %   \label{fig:response_speed}
  % \end{minipage}
\end{figure}
