...
\abstract{%
Data exfiltration leveraging the Domain Name System (DNS) poses a significant and persistent cybersecurity threat, particularly for hyperscalers handling massive AI workloads and sensitive client data across distributed cloud environments. DNS's inherent vulnerabilities make it a prime target, with the average data breach costing over \$4.8 million. Detecting and mitigating DNS-based exfiltration in real-time across massively scaled infrastructures is exceptionally challenging. This capstone project developes a novel and scalable framework for real-time prevention of DNS-based data exfiltration, capable of neutralizing even advanced Command-and-Control (C2) techniques. The system features lightweight, agent-based, endpoint-centric enforcement and deep packet inspection via eBPF directly within the Linux kernel. Kernel-resident programs enable raw, per-packet parsing of outbound DNS traffic, while dense neural networks in userspace perform real-time lexical analysis to detect obfuscated or malicious payloads. Active defense is enforced by terminating malicious processes at the source upon detection. Telemetry, including process identifiers and DNS usage patterns, is exported to messasge brokers for dynamic domain blacklisting and cross-node security policy enforcement. Experimental results confirm the framework's horizontal scalability and unparalleled security strength, effectively preventing advanced DNS data exfiltration and disrupting sophisticated C2 attacks. This research has gained recognition from the Linux kernel community and top-tier security conferences, developing the industry’s first solution to combine deep in-kernel inspection, AI-assisted detection, dynamic in-kernel network policy enforcement, and process-level active defense.
}
...
\begin{itemize}[nosep]
    \item Enforce DNS exfiltration security from within the operating system to provide in-line real-time protection directly on the infected node.
    \item Eliminate reliance on external security middleware's and and reduce detection latency by shifting enforcement to the Linux kernel, where DNS traffic can be inspected and acted upon immediately.
    \item Instantly terminate malicious implants and DNS-based C2 stealthy channel activity, improving both detection accuracy and response time without requiring manual intervention.
    \item Enable advanced data obfuscation detection and adaptive threat response by integrating a userspace deep learning model with in-kernel enforcement logic.
    \item Protect distributed environments by enforcing security policies across nodes, enabled via threat events streaming for scaled infrastructure in real-time.
\end{itemize}
...
Within the network stack, Traffic Control (TC) is a critical subsystem to enforce egress security filtering beyond flow control and quality of service (QoS). TC supports fine-grained control via shaping, scheduling, classification, policing, and drop. These operations are handled through queueing disciplines (QDISCs), which dictate how packets are scheduled and transmitted to the network driver transmission queues \cite{salim2015linux}.
Among QDISCs, CLSACT (classless QDISC with actions) supports features for enhanced security enforcement. It supports packet classification and actions on both ingress and egress without altering existing classful (e.g., HTB) or classless (e.g., FQ\_CODEL, MQ\_PRIO) traffic setups. This compatibility allows CLSACT to seamlessly integrate into production networks, while enabling programmable, in-kernel packet control. eBPF programs attached via CLSACT filters can execute chained classification and action logic with configurable priorities, enabling the kernel to invoke a complete filtering chain based on priority values \cite{borkmann2016getting}.
Since CLSACT executes before any default QDISC, it is well suited for embedding additional security filtering logic without altering existing TC behavior. Its in-kernel enforcement preserves quality of service (QoS) integrity while enabling real-time packet filtering. Despite its adoption by Container Network Interface (CNI) plugins in Kubernetes for overlay routing, IP masquerading, and node-to-node communication, its potential for enforcing in-kernel security policies remains underutilized.
Beyond TC, eBPF programs can attach to multiple layers of the kernel network stack: the high-speed ingress datapath (via XDP), link layer (via netfilter), sockets (via sockops and sk\_msg hooks), and even syscall and LSM layers. This comprehensive hook coverage over core kernel subsystems allows eBPF to support profiling, observability, rate limiting, threat detection, and DPI, all with minimal overhead and maximum control. 
...
DNS-based data exfiltration is a covert technique that extracts sensitive information from compromised systems using the DNS protocol. Frequently used by memory-resident fileless implants, this method minimizes forensic footprints by abusing the ubiquity and permissiveness of DNS, which is rarely filtered or blocked.
Attackers encode payloads into the subdomain section of outbound DNS queries, sending them to attacker-controlled domains or delegated nameservers using standard recursive resolution paths. As shown in Table~\ref{dns_payload_obfuscation}, a variety of obfuscation techniques can be applied to embed and disguise exfiltrated data. These include base encoding, compression, and segmentation.
Common DNS record types such as \texttt{A}, \texttt{AAAA}, \texttt{MX}, and \texttt{HTTPS} are often used due to widespread acceptance, while \texttt{TXT} and \texttt{NULL} are favored for their ability to carry arbitrary payloads - ideal for command-and-control (C2) responses that contain attacker instructions.
To evade detection, adversaries employ domain generation algorithms (DGAs), randomized query timing, and ephemeral key encryption. Some frameworks, such as DNSCat2, tunnel DNS over arbitrary transport ports, encapsulating non-DNS traffic, and bypassing traditional inspection and firewall filters.
Figure~\ref{sec:dns c2 flow} outlines the typical phases of DNS-based exfiltration. The three dominant forms are tunneling, DNS-based C2, and raw exfiltration and are examined in detail below.
...
Several standardized enhancements improve DNS integrity and privacy:
	\begin{itemize}[nosep]
	    \item \textbf{DNSSEC} Add cryptographic signatures to DNS records to ensure authenticity and prevent spoofing or cache poisoning. However, it does not encrypt payloads, leaving queries visible to intermediaries and vulnerable to covert channels or data exfiltration hidden within legitimate DNS traffic.
...
DNS server-side solutions like EXPOSURE \cite{bilge2011exposure} and NOTOS \cite{antonakakis2010building} rely on passive analysis of large datasets, extracting domain features to flag malicious activity. While effective for identifying botnet C2 or spamming domains, they lack the ability to enforce policy or block payload execution in real time. Other models like those from \citeauthor{DBLP:journals/corr/abs-1709-08395} and \citeauthor{10.1145/3230833.3233278}—use entropy, timing, and anomaly models to detect low-throughput tunneling. Yet these approaches are inherently reactive, relying on historical traffic and failing against slow, stealthy exfiltration.

Some solutions attempt mitigation using mobile agents \cite{9486400} or package monitoring in distributed systems \cite{haider2024c2}, but these strategies introduce latency, suffer from false positives, and lack scalability. Even promising tools like Process DNS \cite{sivakorn2019countering} detect C2 traffic by correlating userspace processes, yet remain vulnerable to privilege escalation and evasion due to limited kernel integration and lack of fine-grained kernel enforced mandatory access control or these security tools in userspace.

Overall, most ML-based DNS security tools are limited by userspace-only architectures. They lack in-kernel inspection, cross-protocol correlation, and visibility into port-layer obfuscation—such as DNS over nonstandard ports. These tools operate passively, disconnected from real-time enforcement, and offer no preemptive response to emerging threats. While lexical payload analysis may help classify anomalies quickly, behavioral models still lag behind, detecting threats only after execution or data exfiltration has occurred.

Current designs fundamentally fail to prevent advanced C2 behavior such as remote code execution, port forwarding, or shell access. They rarely address real-world adversary emulation or modern C2 vectors, and remain ineffective against multiplayer C2 operations, botnet-based attacks, or domain generation algorithms (DGA). Despite incremental progress, no existing solution offers real-time DNS exfiltration prevention with implant termination, dynamic in-kernel security policy enforcement, negligible or zero data loss, adaptive domain blacklisting, and cloud-native scalability. Userspace-only systems lack the ability to introspect low-level system state—especially near the NIC—and instead rely on passive traffic analysis from centralized vantage points. This limits their ability to enforce fine-grained controls needed for modern threat defense. Real-time, kernel-level defenses are essential to ensure data sovereignty and containment with the speed and precision today’s advanced threats demand.
...
The data plane is composed of eBPF agents, implemented entirely in Golang for high performance and low memory overhead, and deployed across all endpoint nodes. Operating in userspace, these agents dynamically inject eBPF programs into the kernel’s TC layer at the egress hook of each physical network interface, enabling in-kernel DPI to detect and filter exfiltrated DNS traffic, primarily over UDP. Beyond TC filters, agents also deploy auxiliary eBPF programs at several kernel hook points: kprobes to monitor the creation of new network devices, raw tracepoints to track process termination, and cgroup socket hooks to retrieve internal kernel process representation (task\_struct) on legacy kernels (<5.2) where direct access from TC is not supported. Integration with Linux Security Modules (LSM) ensures the integrity of all injected eBPF programs, protecting the kernel against tampering or malicious modification. Each agent supports two configurable prevention modes, controlled via a dedicated eBPF map injected into the kernel. These modes can be dynamically toggled at runtime from userspace. By default, both are enabled to provide maximum protection against DNS exfiltration attack vectors.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{UWThesis/images/ebpf_network_topology.png}
...
This mode is implemented through several components. \hyperref[sec:dp_eBPF_LRU_Maps_active]{Maps} describes the structure and role of eBPF LRU hash maps (shown in \hyperref[sec:dp_eBPF_LRU_Maps_active]{Figure 4.2}), which track the security state and current policy status enforced by both kernel eBPF programs and userspace eBPF agents. Next, \hyperref[active:sec1]{Kernel eBPF filter packet classification} explains how incoming packets are classified in the TC egress filter using SKB metadata, and how redirection is handled after deep scans by userspace for enhanced security checks. \hyperref[active:sec3]{Userspace eBPF agent packet handling} then details how sniffed packets are processed in zero-copy mode. Finally, \hyperref[active:sec3]{Concurrency handling} discusses safe access and updates to shared maps between kernel and userspace threads, even under parallel execution across multiple CPU cores.
...
\label{active:sec1}
 Once a DNS packet is written to a UDP socket, it traverses the kernel network stack and reaches the TC layer, where classification and shaping occur under the parent QDISC of the relevant network interface. Before any QDISC processing, the kernel triggers the CLSACT QDISC, where the eBPF direct-action filter is attached. The eBPF program then inspects the packet by peeling off L2–L4 headers using the kernel’s native protocol structures. Since DNS is a Layer 7 protocol handled in userspace, its payload appears as raw bytes in the kernel. To parse this, the eBPF program uses custom C structures aligned with RFC 1035 to extract fields like query ID, opcode, flags, question/answer count, and then parses the first DNS question.
If the packet is seen for the first time, there will be no entry for its query ID in the \texttt{dns\_packet\_redirection\_map}. The program then extracts features from the DNS question. If multiple questions are detected—anomalous in most DNS traffic—the packet is flagged and redirected, as legitimate servers typically reject such queries. The answer count is also expected to be zero, since C2 implants do not operate full DNS servers and instead encode commands in DNS questions or the additional section.
These extracted features are evaluated against the kernel feature set described in \hyperref[sec:feature-kernel]{Table 4.3}, and based on the evaluation, appropriate TC actions are enforced, such as \texttt{TC\_ACT\_SHOT} to drop, \texttt{TC\_ACT\_OK} to forward, or \texttt{bpf\_redirect} to redirect the packet to a different network interface. If the packet is benign or malicious the action is taken immediately according to the \hyperref[sec:alg1]{Algorithm 1}. For suspicious packets requiring deeper inspection, before redirection to the different netdev, the parsed DNS query ID is stored as a key in the \texttt{dns\_packet\_redirection\_map} map, with the value being the current kernel system monotonic timestamp obtained via \texttt{bpf\_ktime\_get\_ns}, providing nanosecond precision to accurately track the time of redirection and detect possible timing-based evasion. At this point, additional map information populated by the eBPF agent in userspace during startup is fetched, including the recorded L3 address of the bridge interface to which the packet will be redirected, the interface network \texttt{if\_index}, and other system observability maps. Redirection counters are also incremented for the eBPF agent to export system observability metrics. Prior to redirection, the eBPF program determines whether the incoming packet is IPv4 or IPv6; if IPv4, the program performs destination NAT (DNAT) inside the kernel to modify the L3 destination address and incremental checksum calculation for Ipv4 packets to point to the userspace-owned bridge for inspection, while for IPv6, due to the absence of an L3 checksum in protocol, a static checksum is assigned to maintain consistency. The suspicious packet is then redirected to the RX queues of the associated netdev created by the userspace eBPF agent. 
When the userspace eBPF agent completes deeper inspection and sends the packet back through an \texttt{AF\_PACKET} socket, the TC eBPF program is re-triggered. To prevent infinite loops or reprocessing of malicious re-sends, the eBPF program re-parses the DNS protocol from the SKB, fetches the associated timestamp from the \texttt{dns\_packet\_redirection\_map} map using the DNS query ID as key, and retrieves the scan verification flag from the \texttt{dns\_redirect\_ts\_verify\_map}. If the flag is set, authorizing the packet as legitimately scanned and returned by the eBPF agent, the packet is forwarded toward the NIC and exits the kernel. If the flag is unset, suggesting the packet is forged, unsanctioned, or a result of a timing or brute-force attack by a compromised implant in userspace, the packet is dropped immediately by the eBPF program. The detailed algorithm for active redirection timestamp handling and userspace resend verification are described in \hyperref[sec:alg2]{Algorithm 2}. Through strict enforcement of privileged map access and precise use of the kernel monotonic clock, the system robustly prevents timing attacks, brute-force attempts, and unauthorized packet exfiltration while deep inspection is performed on the packet, involving context switching between userspace and kernel.


\subsubsection{\textbf{Userspace eBPF agent packet processing}}
\label{active:sec2}
The eBPF programs in this mode rely on global (non-per-CPU) kernel maps to support concurrent access from multiple CPUs. These maps use BPF spinlocks—NUMA-aware extensions of kernel spinlocks—to ensure atomic updates and cache coherence per CPU. Each shared map tracks atomic reference counts for access coordination. On the userspace side, the eBPF agent is the sole process responsible for creating and managing map file descriptors. Since map FDs are not shared across processes, this guarantees exclusive ownership and prevents premature garbage collection. Within this agent, concurrent threads that sniff packets or update shared maps synchronize access using userspace \texttt{RWMutex} locks.
This dual concurrency model—BPF spinlocks in the kernel and \texttt{RWMutex} in userspace—ensures consistent parallel processing across CPUs. Critical maps such as \texttt{dns\_packet\_redirection\_map} and \texttt{dns\_redirect\_ts\_verify\_map} use unique keys (DNS query IDs and monotonic timestamps), eliminating race conditions and stale reads that could otherwise leak malicious packets. All kernel-side updates to these maps are performed using LLVM concurrency helpers to ensure atomicity and proper memory synchronization across CPU cores. This architecture provides strict control over data integrity and guarantees that shared kernel–userspace state remains consistent during high-throughput packet inspection.
The complete data flow between userspace and kernel in this mode is illustrated in \hyperref[sec:dp-active-phase]{Figure 4.3}.
...
\If{not \texttt{dns\_kernel\_redirect\_val}}{
  ...
  \If{not \texttt{skb\_mark}}{
    \tcp{Custom skb->mark per netflow computed in-kernel; not set by eBPF agent.}
    ...
    Update \texttt{skb\_netflow\_integrity\_verify\_map} with:
    \begin{itemize}[nosep]
        \item Key: 0xFFEF \tcp{unique flow index identifier for skb}
        \item Value: \texttt{skb\_mark}
    \end{itemize}
  }
  Mark \texttt{skb->mark} = \texttt{skb\_mark}\;

  Update \texttt{dns\_packet\_redirection\_map} with:
  \begin{itemize}[nosep]
  ...
  Update \texttt{redirect\_count\_map} with:
  \begin{itemize}[nosep]
      \item Key: \texttt{if\_idx}\;
      \item Value: updated\_redirect\_count\;
  \end{itemize}
...
% alg 3
\begin{algorithm}[H]
\caption{User-Space eBPF Agent with Deep Learning and Event Streaming for Deep Parsing of DNS Traffic}
\label{sec:alg3}
\SetKwInOut{Input}{Input}
\SetKwInOut{Output}{Output}
...
\Else{
  Fetch \texttt{isBlacklistedSLDFound} from eBPF agent's LRU map\;
...
\If{\texttt{shouldRetransmit}}{
  Fetch \texttt{dns\_kernel\_redirect\_val} = \{\texttt{l3\_checksum}, \texttt{kernel\_time\_ns}\} from \texttt{dns\_packet\_redirection\_map} using \texttt{tx\_id}\;
  Extract \texttt{kernel\_time\_ns} from \texttt{dns\_kernel\_redirect\_val}\;
...
\label{sec:passive}
Passive mode reuses the same eBPF program attached to the egress CLSACT TC filter, as introduced in \hyperref[sec:active]{active mode}, and attaches it to all physical network interfaces at the endpoint. This mode extends the design by layering additional telemetry and process-aware enforcement.
...
Key: Process ID extracted from kernel task\_struct \\
Value: Process detected as malicious or not.
\end{enumerate}
...
The eBPF agent implements passive mode using concurrent goroutines, each pinned to a dedicated OS thread, and operating within an isolated Linux namespace and its associated physical NIC. These goroutines run in zero-copy userspace mode and inspect packets redirected from the kernel eBPF program. Each redirected packet includes both application and transport-layer headers. Upon sniffing the packet redirected by kernel program to different netdev, the agent parses the payload to identify embedded DNS structures. If none are found, the corresponding entry in src\_port\_task\_struct\_map is removed using the source port as the key. If a DNS layer is detected, the agent invokes the same classification pipeline as in active mode—first consulting a local LRU cache of blacklisted domains, then extracting DNS features and passing them to a deep learning model. If the model flags the packet as malicious containing exfiltrated data, it updates the blacklist and emits a threat event to the observability backend. Because the packet is a clone generated by SKB clone , it is never reinjected into the network. Instead, the agent uses the source port to resolve the process and thread IDs from task\_struct\_redirect\_ct\_map. It then flags the process in malicious\_process\_map, allowing the kernel to drop future packets from it in real time. The agent also inspects redirect telemetry—specifically the count of clone-redirected packets per process. If the count exceeds a configured threshold and the process is already marked malicious, the agent, running with CAP\_SYS\_ADMIN, terminates the process. This forms a coordinated, process-aware security strategy between kernel and userspace, targeting stealthy DNS exfiltration over arbitrary UDP ports. Algorithm 6 details the full userspace processing flow.

\subsubsection{\textbf{eBPF Maps concurrency handling}}
\label{passive:sec3}
\vspace{-1em}
This mode mirrors active mode’s concurrency principles by using per-CPU kernel spin locks on shared eBPF maps, ensuring updates persist as long as the userspace agent runs. For \texttt{src\_port\_task\_struct\_map}, atomicity is achieved via unique src\_port keys, enabling kernel writes and userspace reads without race conditions. Userspace updates to \texttt{malicious\_process\_map} use the \texttt{BPF\_ANY} flag and are synchronized with a mutex since kernel programs only read from this map, enabling RCU-style performance. For \texttt{task\_struct\_redirect\_ct\_map}, where kernel code writes and userspace reads, spin locks protect kernel writes, while a separate mutex guards concurrent userspace reads.\hyperref[sec:dp-passive-phase]{Figure 4.5} shows the full TC pipeline.
...
\If{not \texttt{is\_malicious} or \texttt{is\_malicious} is \texttt{null}}{
    Update \texttt{src\_port\_task\_struct\_map}:
    \begin{itemize}[nosep]
        \item Key: \texttt{process\_id}
    ...
    Update \texttt{task\_struct\_redirect\_ct\_map}:
    \begin{itemize}[nosep]
        \item Key: \texttt{task\_struct}
    ...
    Update \texttt{dns\_packet\_clone\_redirection\_ct\_map}:
    \begin{itemize}[nosep]
        \item Key: \texttt{if\_index}
    ...
    \If{\texttt{is\_malicious} is \texttt{null}}{
        ...
        Update \texttt{malicious\_process\_map}:
        \begin{itemize}[nosep]
            \item Key: \texttt{process\_id}
            \item Value: \texttt{false}
        \end{itemize}
    }
...
\Else{
    ...
    Perform \texttt{bpf\_clone\_redirect(skb, bridge\_if\_index, BPF\_F\_INGRESS)}\;
    \Return \texttt{TC\_ACT\_SHOT}\;
    \tcp{Drop packet — malicious process attempted exfiltration. Continue redirecting clones to monitor retry behavior.}
}
\end{algorithm}
...
\If{\texttt{isBlacklistedSLDFound}}{
    ...
    \If{\texttt{current\_suspicious\_ct} $>$ \texttt{MAX\_MALICIOUS\_THRESHOLD}}{
        ...
        Delete from \texttt{malicious\_process\_map} with:
        \begin{itemize}[nosep]
            \item Key: \texttt{process\_id}
        \end{itemize}
    }
...
\caption{eBPF Agent DNS Exfiltration Prevention Flow for Process-Aware Adaptive Mode}
...
The current implementation of the eBPF agent focuses on L2-level encapsulation over software network devices, not on tunnels that rely on kernel cryptographic primitives via the keyring, such as those used by OpenVPN, IPsec, or WireGuard. This design choice aligns with the typical behavior of the DNS protocol, as DNS resolution rarely occurs over VPN tunnels. In this context, DNS exfiltration over encapsulated traffic is primarily limited to VLAN and TUN/TAP software network devices.
...
\hline
\texttt{exfil-sec} & Kafka topic used by the eBPF agentss in data plane to stream prevented DNS threat events. \\
\hline
...
The complete security framework codebase—including all configuration files and documentation—is publicly available at \href{https://github.com/Synarcs/DNSObelisk}{GitHub}. It is currently licensed under AGPLv3, with plans to migrate to GPL as part of a broader effort to incubate the project under the Linux Foundation.
...
Comprehensive setup instructions are included in the Documentation folder.
...
Complementing kernel profiling, the userspace eBPF agent—responsible for traffic handling—was profiled using Go’s \texttt{pprof} tool. Analysis of CPU-intensive function call stacks revealed that approximately 75\% of the agent’s CPU usage is spent in BPF-related syscalls, reflecting its reliance on frequent kernel interactions to access and update eBPF program internals, particularly eBPF maps. \hyperref[fig:c4]{Figure 7.1} presents the agent’s flame graph.
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{DNS Exfiltration Prevention Metrics: TUN/TAP kernel Network encapsulation and Latency}
\end{figure}
...
\subsection{Protecting Linux Kernel from malicious eBPF programs}[htbp]
...
To mitigate this risk, additional eBPF programs are loaded into the kernel and attached to LSM hooks that intercept BPF syscalls, including  \texttt{BPF\_PROG\_LOAD}. These LSM programs verify digital signatures on incoming eBPF bytecode, following a process analogous to the kernel’s verification of loadable modules.
...
Furthermore, the core security layer integrates with a centralized control plane connected to the cloud Public Key Infrastructure (PKI), enabling a scalable layered trust model — from cloud PKI to kernel-level mandatory access control. While the keyring currently stores sensitive keys in the software, the kernel restricts access to unprivileged userspace processes. Future enhancements include integration with Trusted Platform Modules (TPMs) or Hardware Security Modules (HSMs), widely supported by cloud providers, leveraging hardware-based firmware security guarantees. The core implementation algorithm for the kernel-resident LSM-based eBPF verification program is detailed below.
