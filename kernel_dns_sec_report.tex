...
\titleformat{\section}
  {\bfseries\large}{\thesection}{0.75em}{}

\titleformat{\subsection}
  {\bfseries\normalsize}{\thesubsection}{0.5em}{}

\titleformat{\subsubsection}
  {\bfseries\small}{\thesubsubsection}{0.3em}{}
...
\Year{2025}
\Program{Computing \& Software Systems}
...
combines Linux network namespaces with multiple pairs, acting as Layer 2 and Layer 3 bridges. Agents also maintain file descriptors to their eBPF maps, enabling efficient kernel-userspace communication and advanced runtime state analysis. The lifecycle of each eBPF program is tightly managed by the agent, which operates with elevated privileges to control the kernel network stack, syscalls, and the relevant kernel subsystems. Some of the kernel capabilities required for the agent are detailed in Table~\ref{sec:dp_kernel_cap}. Both prevention modes, active and passive, support real-time enforcement, including the termination of processes responsible for repeated exfiltration beyond configurable thresholds.
...
\begin{table}[htbp]
\centering
\caption{eBPF Agent: Injected eBPF programs inside Linux Kernel}
\resizebox{\textwidth}{!}{%
...
\caption{eBPF Agent: Network Topology at Endpoint}
...
\section{Data Plane}
The eBPF agent implementation deployed on each node in the data plane is organized into six core components. First, Section~\ref{sec:active} introduces the \textit{strict enforcement mode}, in which suspicious DNS traffic is redirected to userspace and dropped in real time if classified as malicious. Second, Section~\ref{sec:passive} presents an \emph{passive process threat-hunting strategy} that correlates DNS exfiltration with the originating malicious userspace process. Third, Section~\ref{sec:encap} addresses \emph{DNS exfiltration hidden within kernel-encapsulated traffic}, which is currently prevented only in active mode. Fourth, Section~\ref{sec:features} outlines the \emph{features extracted} in the kernel for filtering, and in userspace for deep learning--based inference. Fifth, Section~\ref{sec:dataset} details the \textit{data sources} used to train and evaluate the detection models. Finally, Sections~\ref{sec:model} and~\ref{sec:threat-event-streaming} describe the \emph{model architecture}, including serialization and quantization, as well as the \emph{streaming pipeline} for threat events from the agent to centralized message brokers and observability backends.
...
This mode implementation includes several key components. \hyperref[sec:dp_eBPF_LRU_Maps_active]{\emph{Maps}} describes the structure and purpose of the eBPF LRU hash maps (see Figure~\ref{sec:dp_eBPF_LRU_Maps_active}). These maps track the per-packet security state and current policy status, enforced by both the kernel eBPF programs and userspace eBPF agent. Next, \hyperref[active:sec1]{\emph{Kernel eBPF filter packet classification}} describes the classification of outgoing packets at the TC egress filter using SKB metadata, along with the corresponding TC actions applied to malicious and benign DNS packets. \hyperref[active:sec3]{\emph{Userspace eBPF agent packet handling}} presents the zero-copy processing of sniffed packets for efficient analysis. Finally, \hyperref[active:sec3]{\emph{Concurrency handling}} outline the safe access and update mechanisms for shared eBPF maps between eBPF programs inside kernel and user space agent threads.
...
\Output{
  Packet action: \texttt{TC\_ACT\_SHOT}, \texttt{TC\_ACT\_OK}; \\
  eBPF map updates: \texttt{bpf\_map\_updates}
}

\tcp{Parse skb layers; ensure \texttt{skb->data\_ptr} remains memory safe}
Parse Layer 2 (Ethernet) from \texttt{skb}\;
\If{VLAN (802.1Q or 802.1AD) is present}{
    \If{\texttt{skb->data\_ptr} exceeds \texttt{skb->data\_end}}{
        \Return \texttt{TC\_ACT\_SHOT}\;
    }
    Extract inner encapsulated protocol (\texttt{h\_proto})\;
}
...
\If{\texttt{udp->dest} $\neq$ 53 \textbf{and} $\neq$ 5353 \textbf{and} $\neq$ 5355}{
    \tcc{Not standard DNS/MDNS/LLMNR}
    \Return \texttt{TC\_ACT\_OK}\;
}
...
\tcc{Label Count Check}
\If{\texttt{n\_lbls} $\leq$ 2}{
    \Return \texttt{TC\_ACT\_OK}\;
}

\tcc{Deep Inspection Based on Limits}
\If{\texttt{dom\_len} $>$ \texttt{dns\_limits.dom\_threshold} \textbf{or} 
     \texttt{subdom\_len} $>$ \texttt{dns\_limits.subdom\_threshold}}{
    Perform \texttt{bpf\_map\_updates}\;
    \Return \texttt{TC\_ACT\_SHOT}\;
}

\tcc{Mark packet for userspace processing}
Redirect packet via \texttt{dns\_packet\_redirection\_map} to userspace\;
\Return \texttt{TC\_ACT\_OK}\;
...
% alg 2
\begin{algorithm}[H]
\caption{eBPF Map Handling in \textbf{Active} Mode of Agent}
\label{sec:alg2}
\SetKwInOut{Input}{Input}
\SetKwInOut{Output}{Output}
...
\If{not \texttt{dns\_kernel\_redirect\_val}}{
  \tcc{Packet intercepted first time by TC eBPF filter}
  Modify \texttt{skb} destination IP to \texttt{dst\_ip}\;
  ...
  \If{not \texttt{skb\_mark}}{
    Set \texttt{skb\_mark} = \texttt{bpf\_get\_prandom\_u32}\;
  ...
  \tcc{Global packet redirect count metric for each associated netdev}
  Increment and update \texttt{redirect\_count\_map} with key=\texttt{if\_idx}, value=\texttt{new\_packet\_redirect\_count}\; 
  Perform \texttt{bpf\_redirect(bridge\_if\_index, BPF\_F\_INGRESS)}\;
}
\Else{
  \tcc{Userspace deep-scanned packet re-arrived; verify it is not forged}
  ...
  \If{not \texttt{redirect\_ts\_verify\_val}}{
    \tcc{Timing attack: userspace agent did not emit packet}
    \Return \texttt{TC\_ACT\_SHOT}\;
  }
  \Else{
    \tcc{Packet rescanned from authorized sender, clean required map entries}
...
\Output{%
  Packet either garbage collected (if malicious) or retransmitted (if benign)
}

Sniff traffic over veth pair interfaces in Linux namespace\;

Extract DNS features from sniffed packets\;

Export monitoring metrics (e.g., \texttt{redirection\_count}, \texttt{loop\_time}) from kernel maps\;

Fetch \texttt{isSLDBenign} from userspace LRU map of top 1M SLDs\;

\uIf{\texttt{isSLDBenign}}{
  Set \texttt{shouldRetransmit} $\leftarrow$ \texttt{true}\;
}
\Else{
  Fetch \texttt{isBlacklistedSLDFound} from malicious SLD map\;
  
  \If{\texttt{isBlacklistedSLDFound}}{
    \Return \tcc*[r]{Drop packet immediately}
  }

  Pass DNS features to ONNX model for inference\;

  \If{inference result is \texttt{MALICIOUS}}{
    Emit event to message brokers with packet details\;
    Blacklist SLD in malicious map\;
    Garbage collect packet data\;
    \Return
  }
  \ElseIf{inference result is \texttt{BENIGN}}{
    Set \texttt{shouldRetransmit} $\leftarrow$ \texttt{true}\;
  }
}

\If{\texttt{shouldRetransmit}}{
  Fetch \texttt{dns\_kernel\_redirect\_val} = \{\texttt{l3\_checksum}, \texttt{kernel\_time\_ns}\} from \texttt{dns\_packet\_redirection\_map} by \texttt{tx\_id}\;
  Extract \texttt{kernel\_time\_ns}\ from \texttt{dns\_kernel\_redirect\_val};

  \If{\texttt{AF\_PACKET}}{
    Update \texttt{dns\_redirect\_ts\_verify\_map} with key \texttt{kernel\_time\_ns}, value \texttt{true}\;
    Replace packet’s \texttt{l3\_checksum}\;
    Serialize packet payload to raw bytes\;
    Write packet via \texttt{syscall.write(AF\_PACKET, SOCK\_RAW, 0)}\;
  }
  
  \If{\texttt{AF\_XDP}}{
    Delete \texttt{kernel\_time\_ns} from \texttt{dns\_redirect\_map}\;
    Serialize packet payload to raw bytes\;
    Write packet via \texttt{syscall.write(AF\_XDP, SOCK\_RAW, 0)}\;
  }
}
\end{algorithm}
...
The passive mode architecture is broken down as follows. \hyperref[sec:maps]{Maps} introduces the key eBPF structures used in this mode, their types, and their roles, as visualized in Figure~\ref{sec:dp_eBPF_LRU_Maps_passive}. \hyperref[passive:sec1]{\emph{Kernel packet processing}} details how the eBPF programs in the kernel identify and drop malicious DNS exfiltration attempts while tracking the responsible userspace processes. It also leverages kernel tracepointsp attached to kernel process scheduler—for efficient garbage collection of malicious process tracking. The next section, \hyperref[passive:sec2]{\emph{Userspace packet processing}}, describes how the eBPF agent parses and classifies traffic to identify potentially malicious processes involved in exfiltration. Finally, \hyperref[passive:sec3]{\emph{eBPF map concurrency}} explains how the kernel and userspace components maintain consistent eBPF map state under concurrent CPU access.
...
Because the kernel eBPF program cannot reliably determine whether application data over arbitrary UDP ports contains DNS, it uses \texttt{skb\_clone\_redirect} to create a full copy of packets that heuristically resemble DNS. These clones are redirected to a netdev interface managed by the eBPF agent. To minimize memory overhead, redirection is only performed if the payload structure heuristically aligns with DNS protocol fields. The eBPF program performs raw parsing within SKB bounds (skb-$>$data\_end) and validates structure against constraints defined in RFC 1035. If the payload passes these checks, clone redirection is triggered. The DNS protocol parsing as explained before from application data of other protocols in SKB is explained in Algorithm~\ref{sec:alg4}.

Before redirection, the TC eBPF program uses \texttt{bpf\_get\_current\_pid\_tgid} to fetch the current \texttt{task\_struct}, identifying the process and thread responsible for sending the packet. It then checks the \texttt{malicious\_process\_map} to verify whether the process has been flagged as malicious by the userspace eBPF agent due to prior suspicious transfers. If flagged, the kernel drops subsequent packets while still cloning and redirecting packets. This design enables dynamic, process-level enforcement, allowing userspace agent to observe retry behavior, gather telemetry, and ultimately terminate stealthy implants that beacon intermittently and exceed the configured threshold.
...
This process-aware passive mode enables adaptive detection and response for exfiltration over nonstandard ports. The passive processing logic of the userspace eBPF agent, as previously explained, is detailed in Algorithm~\ref{sec:alg6}.
...
\tcc{Verify DNS count fields are within valid range}
\If{%
  \texttt{qd\_count} $\geq 256$ \textbf{or} 
  \texttt{ans\_count} $\geq 256$ \textbf{or} 
  \texttt{auth\_count} $\geq 256$ \textbf{or} 
  \texttt{add\_count} $\geq 256$%
}{
  \Return \texttt{TC\_ACT\_OK} 
}
...
\tcc{Validate opcode and rcode per RFC 1035}
...
% Extract \texttt{if\_index} from \texttt{skb}\;
% Fetch \texttt{dst\_ip} and \texttt{bridge\_if\_index} from \texttt{netlink\_links\_config} using \texttt{if\_index}\;
...
Extract DNS query ID (\texttt{tx\_id}) from DNS header\;

\tcc{\texttt{if\_index} is netdev link index in kernel}
...
Fetch \texttt{is\_malicious} flag from \texttt{malicious\_process\_map} using \texttt{process\_id}\;

\If{not \texttt{is\_malicious} or \texttt{is\_malicious} is \texttt{null}}{
    \tcc{Track src port for the packet and corresponding process transmitting it}
    ...
    \tcc{Suspicious packet redirect count per process}
    Update \texttt{task\_struct\_redirect\_ct\_map} with key=\texttt{task\_struct}, value=\texttt{current\_suspicious\_ct + 1}\;

    \tcc{Global packet clone redirect count metric per netdev}
    Update \texttt{dns\_packet\_clone\_redirection\_ct\_map} with key=\texttt{if\_index}, value=\texttt{new\_clone\_redirect\_count}\;

    \If{\texttt{is\_malicious} is \texttt{null}}{
        \tcc{First DNS packet sent by process; mark initially the process as benign}
...
\Else{
    ...
    \tcc{Drop packet since malicious process attempted exfiltration; continue clone redirecting clones for monitoring malicious behavior}
    Perform \texttt{bpf\_clone\_redirect(skb, bridge\_if\_index, BPF\_F\_INGRESS)}\;
    \Return \texttt{TC\_ACT\_SHOT}
}
\end{algorithm}
...
\Input{%
  Sniffed DNS packets from suspicious Linux namespaces via \texttt{pcap}; \\
  all kernel eBPF maps; \\
  eBPF LRU hash maps: \texttt{malicious\_process\_map}, \texttt{src\_port\_task\_struct\_map}, \texttt{task\_struct\_redirect\_ct\_map}
}
...
\If{DNS layer \textbf{not} present in \texttt{skb->data}}{
    \Return\tcc*{Ignore non-DNS traffic}
}
...
Extract \texttt{process\_id} and \texttt{thread\_group\_id} from \texttt{task\_struct}\;

Fetch \texttt{isBlacklistedSLDFound} from userspace LRU hash\;

\If{\texttt{isBlacklistedSLDFound}}{
    \tcc{Mark the process as malicious}
    ...
    \If{\texttt{current\_suspicious\_ct} $>$ \texttt{MAX\_MALICIOUS\_THRESHOLD}}{
        \tcc{Terminate malicious process and clean maps}
        Send \texttt{SIGKILL} to \texttt{process\_id}\;
        Delete key=\texttt{process\_id} from \texttt{malicious\_process\_map}\;
    }
    \Return;
}

Pass extracted features to ONNX model for inference\;
...
\ElseIf{inference\_result == \texttt{BENIGN}}{
    Garbage collect sniffed cloned packet data\;
    \tcc{No immediate action; track future packets}
    \Return;
}
\end{algorithm}
...
\begin{enumerate}[nosep]
    ...
    Table~\ref{dns_payload_obfuscation}) across various file types—text (e.g., \texttt{.txt}, \texttt{.md}), image (e.g., \texttt{.jpg}, \texttt{.png}), and video (e.g., \texttt{.mp4})
\end{enumerate}
...
\centering
\caption{Kafka Stream Topics utilized by the eBPF agent}
...
Table-~\ref{tab:kafka-topics} explains the details of kafka topics and their significance in the security framework.
Appendix A for additional details on the metrics exported by the eBPF agent.
...
\subsubsection{eBPF Agent Effectiveness over DNS Exfiltration Tools}
...
\begin{figure}[H] 
 ...
 \begin{minipage}[b]{0.95\textwidth}
    \centering
    \includegraphics[width=0.58\textwidth]{UWThesis/images/results/response_speed/response_speed.png}
...
\begin{figure}[H]
  \centering
  % Second: Table as an image
  \begin{minipage}[b]{0.95\textwidth}
    \centering
    \captionof{table}{Framework Coverage Against Real-World DNS C2 and Exfiltration Tools}
    \includegraphics[width=0.85\textwidth]{UWThesis/images/results/C2_exfil_strength.png}
...
\subsection{Distributed Infrastructure}
The performance evaluation focuses on the DNS server, assessing the throughput impact of the Lua-based interceptor in the PowerDNS Recursor. As shown in Figure~\ref{fig:throughput_gsld_tcp}, which plots benchmark duration on the horizontal axis, with throughput and packet loss on the two vertical axes, the server experienced a drop to 490 DNS requests per second under a 10,000 requests/sec load. This degradation stems from reusing the same inference server design as the data plane, reliance on UNIX sockets, and Python’s concurrency limits. Latency results in Figure~\ref{fig:throughput_onnx_tcp} show benchmark duration on the X-axis for a period of 20 seconds and associated latency on the Y-axis, peaking at 750 ms with a mean deviation of 380 ms. All TCP traffic benchmarks used \texttt{TCP\_FAST\_OPEN}, allowing data transmission with the initial SYN packet and reducing handshake overhead for accurate DNS-over-TCP latency measurements. 
Figure~\ref{fig:dns_rpz} displays the blacklisted domains listed in the DNS RPZ table on the server.

% \footnote{Appendix B provides additional details for blacklisted domains DNS RPZ table on DNS server}

% \hyperref[fig:dns_rpz]{Figure 5.16} displays blacklisted domains stored in the PowerDNS GPSQL backend. The controller augments this list with metadata and can trigger reprogramming of all data plane nodes to override heuristics and allow specific DNS traffic.

\begin{figure}[H]
  \centering
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{UWThesis/images/results/tcp/throughput_active_onnx_tcp.png}
  ...
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{UWThesis/images/results/tcp/latency_metrics_onnx_tcp.png}
...
\Input{%
  \texttt{skb} (socket buffer)\newline
  eBPF map: \texttt{skb\_netflow\_integrity\_verify\_map}
}
\Output{%
  \texttt{TC\_ACT\_OK} — packet allowed\newline
  \texttt{TC\_ACT\_SHOT} — drop tampered packet
}

\tcc*[r]{Unique mark used in core TC program on each physical netdev}
...
\If{\texttt{skb\_mark\_integrity\_mark} $\neq$ \texttt{skb->mark}}{
    \tcc{Tampered or spoofed redirection; reject packet}
    \Return \texttt{TC\_ACT\_SHOT}\;
}

\Return \texttt{TC\_ACT\_OK} \tcc*[r]{Packet is legitimate}
\end{algorithm}
...
Algorithm~\ref{sec:alg8} details the algorithm implemented in PowerDNS Recrusor as an interceptor.
...
\If{$dq.isTcp$}{
    ...
    \If{$\text{result}[\texttt{"threat\_type"}]$}{
        ...
        \Return \textbf{true} \tcc*[r]{Block malicious DNS over TCP}
    }
}

\If{$sf\_blacklist.\text{check}(\text{getSLD}(qname))$}{
    $dq.rcode \gets \texttt{NXDOMAIN}$\;
    \Return \textbf{true} \tcc*[r]{Block known SLD from local blacklist}
}

\Return \textbf{false} \tcc*[r]{Allow query if not malicious}
\end{algorithm}
