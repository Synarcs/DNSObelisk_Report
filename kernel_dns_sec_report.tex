...
This project develops a novel scalable framework for real-time DNS exfiltration prevention, designed to counter advanced Command-and-Control techniques. The framework uses lightweight eBPF agents to perform deep packet inspection directly in the Linux kernel, enabling high-speed, per-packet analysis of outbound DNS traffic. A userspace neural network performs real-time lexical analysis to detect obfuscated or malicious payloads. Upon detection, the kernel enforces active defense by immediately terminating malicious processes to contain the threat at the source. The system also exports telemetry, including process identifiers and DNS usage patterns, to Kafka brokers, supporting dynamic domain blacklisting and policy enforcement across nodes without relying on external middlewares.
Experimental validation demonstrates that the framework effectively identifies and terminates advanced DNS exfiltration techniques with subsecond response times, including attacks from prominent red team frameworks. This rapid response reduces attacker dwell time, prevents lateral movement, and enhances threat visibility for security teams managing distributed environments.
}
...
\caption{DNS Data obfuscation detection Deep Learning Model ONNX Graph}
...
The eBPF agent was evaluated across all data plane endpoints against widely used DNS-based C2 frameworks employed by red teams for penetration testing. Tests focused on disrupting command-and-control and exfiltration attempts tunneled through DNS, with the CSSVLAB06 node acting as the attacker.
The BishopFox Sliver C2 framework  was tested in active mode for both basic exfiltration and advanced operations, including remote shell access, code execution, file transfers, port forwarding, and persistence—all over DNS. The agent intercepted the first C2 packet at the kernel level, immediately disrupting communication, forcing retries, and terminating implants for both beacon- and session-based channels.
While Sliver lacks support for randomized UDP ports, transport obfuscation was evaluated using dnscat2. The agent, in both active and passive modes, successfully enforced mitigation policies, resulting in full-session interruption with zero data exfiltration. The average response time for the eBPF agent to make a decision and drop an exfiltration attempt was 316.233 microseconds, as shown in Figure~\ref{fig:response_speed}, which presents per-exfiltration response times before implant termination.
For raw exfiltration scenarios, tools like DET were tested and blocked instantly. Additionally, iodine was evaluated for protocol tunneling via TUN/TAP interfaces; the agent effectively dropped all encapsulated payloads. Other bulk exfiltration tools were similarly thwarted. Table~\ref{tab:dns-framework-coverage} summarizes all tools and blocked vectors.
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[b]{0.95\textwidth}
    ...
    \caption{eBPF Agent DNS Exfiltration Attempt Response Time}
    \label{fig:response_speed}
  \end{minipage}

  \vspace{1em} % add a little vertical space between image and table

  \begin{minipage}[htbp]{0.95\textwidth}
    \centering
...
The performance evaluation focuses on the DNS server, assessing the throughput impact of the Lua-based interceptor in the PowerDNS Recursor. As shown in \hyperref[fig:throughput_gsld_tcp]{Figure 5.14}, which plots benchmark duration on the horizontal axis, with throughput and packet loss on the two vertical axes, the server experienced a drop to 490 DNS requests per second under a 10,000 requests/sec load. This degradation stems from reusing the same inference server design as the data plane, reliance on UNIX sockets, and Python’s concurrency limits. Latency results in \hyperref[fig:throughput_onnx_tcp]{Figure 5.15} show benchmark duration on the X-axis for a period of 20 seconds and associated latency on the Y-axis, peaking at 750 ms with a mean deviation of 380 ms. All TCP traffic benchmarks used \texttt{TCP\_FAST\_OPEN}, allowing data transmission with the initial SYN packet and reducing handshake overhead for accurate DNS-over-TCP latency measurements. \hyperref[fig:dns_rpz]{Figure 5.16} displays blacklisted domains stored in the PowerDNS GPSQL backend. The controller augments this list with metadata and can trigger reprogramming of all data plane nodes to override heuristics and allow specific DNS traffic.
