...
\If{not \texttt{dns\_kernel\_redirect\_val}}{
  ...
  Mark \texttt{skb->mark} = \texttt{skb\_mark}\;
    ...
    Update \texttt{dns\_packet\_redirection\_map}:
    \begin{itemize}[nosep]
        \item Key: \texttt{tx\_id}
...
\Else{
  ...
  \Else{
    \tcp{Packet rescanned from authorize sender forward the packet}
    Delete \texttt{redirect\_ts\_verify\_val} from \texttt{dns\_redirect\_ts\_verify\_map}\;
    Perform \texttt{TC\_ACT\_OK}\; 
  }
}
...
\If{not \texttt{is\_malicious} or \texttt{is\_malicious} is \texttt{null}}{
    ...
    \If{\texttt{is\_malicious} is \texttt{null}}{
        \tcp{Process send the DNS packet first time, the process is not yet found malicious}
        Update \texttt{malicious\_process\_map} with:
        \begin{itemize}[nosep]
            \item Key: \texttt{process\_id}
...
\Else{
    ...
    \tcp{Drop the packet exfiltration attempt found over the process, keep clone redirecting from kernel to monitor implant retry behavior, to be killed}
}
\end{algorithm}
