...
\abstract{%
Data exfiltration remains one of the most persistent and sophisticated threats in cybersecurity, with the Domain Name System (DNS) frequently exploited as a covert channel for tunneling and command-and-control (C2) communications. This threat is especially critical for hyperscalers, where the majority of servers run Linux. This threat is especially critical for hyperscalers, with most servers running Linux, as the surge in AI workloads demands massive data storage while simultaneously hosting sensitive client data at scale, both on-prem and across distributed cloud environments. DNS remains highly vulnerable due to its ubiquity, inherit security flaws, and its fundamental role in enterprise networks. As highlighted in the IBM 2024 Data Breach Report, the average cost of a data breach now exceeds \$4.8 million, with DNS-based exfiltration posing catastrophic risks that are notoriously hard to mitigate in real time. This paper presents a novel and scalable framework for real-time prevention of DNS-based data exfiltration, including advanced C2 implants, across distributed environments. The system is built around lightweight, agent-based, endpoint-centric enforcement and leverages deep packet inspection through eBPF injected directly inside the Linux kernel’s traffic control layer. These kernel-resident programs enable raw, per-packet parsing of outbound DNS traffic at high speed, aided by dense neural networks in userspace that perform real-time lexical analysis to detect obfuscated or malicious queries. Active defense is enforced directly from within the operating system by terminating malicious processes upon detection, accelerating incident response, and immediately containing compromise at the source.
Telemetry, including process identifiers, lifetimes, and DNS usage patterns, is exported per node to Kafka brokers, enabling dynamic domain blacklisting and DNS-based policy enforcement throughout the network. The architecture supports massive scalability and enforces real-time cross-node security policies without reliance on external security middleware or firewalls. By combining deep in-kernel inspection, AI-assisted detection, dynamic in-kernel network policy enforcement, and process-level active defense, this framework minimizes attacker dwell time, blocks lateral movement, and significantly enhances visibility and response for defenders operating in large-scale, production-grade cloud and on-premise environments.
}
...
\tcp{use the random hash userspace generated for skb integrity verification over bridge post live-redirect}
...
\If{not \texttt{dns\_kernel\_redirect\_val}}{
  ...
  \If{not \texttt{skb\_mark}}{
    \tcp{custom skb\_mark per netflow not inserted from eBPF agent compute unique ID per netflow in kernel}
  ...
  Update \texttt{dns\_packet\_redirection\_map} with key \texttt{tx\_id} and value \{\texttt{l3\_checksum}, \texttt{kernel\_time\_ns}\}\;
  ...
  Update \texttt{redirect\_count\_map} with key \texttt{if\_idx} and updated count\;
...
\Else{
  ...
  Fetch and delete \texttt{redirect\_ts\_verify\_val} from \texttt{dns\_redirect\_ts\_verify\_map} using key \texttt{kernel\_time\_ns}\;
...
\Output{%
  Packet dropped (if malicious) or socket write (if benign)
}
...
Fetch \texttt{isSLDBenign} from eBPF agent's LRU map of top 1M SLDs\;
\If{\texttt{isSLDBenign}}{
  Set \texttt{shouldRetransmit} $\leftarrow$ \texttt{true}\;
}
\Else{
  Fetch \texttt{isBlacklistedSLDFound} from eBPF agent's LRU Hash\;
...
\If{\texttt{shouldRetransmit}}{
  ...
  \If{\texttt{AF\_PACKET}}{
    Update \texttt{dns\_redirect\_ts\_verify\_map} with: \\
    \quad Key: \texttt{kernel\_time\_ns} \\
    \quad Value: \texttt{true}\;

    Replace packet’s \texttt{l3\_checksum}\;
    Serialize packet payload to raw bytes\;
    \texttt{syscall.write(AF\_PACKET, SOCK\_RAW, 0)}\;
  }

  \If{\texttt{AF\_XDP}}{
    Delete \texttt{kernel\_time\_ns} from \texttt{dns\_redirect\_map}\;
    Serialize packet payload to raw bytes\;
    \texttt{syscall.write(AF\_XDP, SOCK\_RAW, 0)}\;
  }
}
\end{algorithm}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{DGA Mitigation with dynamic L3 kernel Network Policies} – Dynamically blacklists  domains, reprogram agents in data plane and enforce layer 3 network policies for cross protocol coorelation.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{UWThesis/images/kernel_bpftop.png}
...
\If{\texttt{skb\_mark\_integrity\_mark} $\neq$ \texttt{skb->mark}}{
    \tcp{This is a tampered redirected packet and did not originate from the main TC egress program}
    \Return \texttt{TC\_ACT\_SHOT}\;
}
...
In order to start the control plane server on controller nodes, ensure the DNS server, Kafka brokers are healthy, and GPSQL storage back-end used by the PowerDNS server is reachable. Below are the steps to start the control server on the controller nodes.
