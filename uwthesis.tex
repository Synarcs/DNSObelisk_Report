...
\If{not \texttt{is\_malicious}}{
    Update \texttt{src\_port\_task\_struct\_map} with \texttt{key=process\_id}, \texttt{value=task\_struct}\;
    Fetch \texttt{current\_suspicious\_ct} from \texttt{task\_struct\_redirect\_ct\_map} using \texttt{task\_struct} as key:
    \begin{itemize}[nosep]
    ...
    \end{itemize}
    Perform bpf_clone_redirect(skb, bridge_if_index, BPF\_F\_INGRESS)
}
\Else{
...
\Output{Real-time updates to \texttt{malicious\_process\_map}}
...
\If{\texttt{isBlacklistedSLDFound}}{
    Update \texttt{malicious\_process\_map} for process ID from \texttt{kernel\_task\_struct}\;
    \Return;
}
...
\If{Inference == \texttt{MALICIOUS}}{
    ...
    Blacklist SLD for related DNS records\;
    Update \texttt{is\_malicious flag} in \texttt{malicious\_process\_map} with key as process ID from \texttt{kernel\_task\_struct}\;
    \Return;
}
